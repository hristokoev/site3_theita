...
<script>
	<?php if (!isset($trial)) { ?>
		const images = [<?php foreach ($media[$usetype] as $k => $photogroup) {
						...
						src: "<?php echo cdn_hook('https://members.vangoren.com' . $GLOBALS["contentdir"] . "upload/" . $photogroup["jpg:$usetype"]["fullpath"]) ?>",
		...
		const images = [
			...
			?> {
						src: "<?php if (in_array($cnt, $trial["clickpages"]) || $trial["clickpages"][0] == 0) {
									echo cdn_hook('https://members.vangoren.com' . $GLOBALS["contentdir"] . "upload/" . $photogroup["watermark:$usetype:$size"]["fullpath"]);
								} ?>",
						width: <?php echo $photogroup["watermark:$usetype:$size"]["width"]; ?>,
						height: <?php echo $photogroup["watermark:$usetype:$size"]["height"]; ?>,
						index: <?php echo $cnt; ?>
				...
				?> {
						src: "<?php if (in_array($cnt, $trial["clickpages"]) || $trial["clickpages"][0] == 0) {
									echo cdn_hook('https://members.vangoren.com' . $GLOBALS["contentdir"] . "upload/" . $photogroup["jpg:$usetype"]["fullpath"]);
								} ?>",
						width: <?php echo $photogroup["jpg:$usetype"]["width"]; ?>,
						height: <?php echo $photogroup["jpg:$usetype"]["height"]; ?>,
						index: <?php echo $cnt; ?>
